...
def buscar_google_jurisprudencia(query: str) -> str:
    """Busca doutrina, artigos acadêmicos e jurisprudência sobre um tema jurídico para fundamentar uma consulta."""
...
class AgenteTecnicoParecer:
    def __init__(self, llm_api_key):
        ...
        self.tools = [Tool(name="BuscaGoogleJurisprudencia", func=buscar_google_jurisprudencia, description="Busca doutrina, artigos acadêmicos e jurisprudência sobre um tema jurídico para fundamentar uma consulta.")]
        
        # --- CORREÇÃO 2/2: ADIÇÃO DA VARIÁVEL {tool_names} ---
        react_prompt_template = """Você é um jurista e parecerista renomado. Sua missão é analisar a consulta jurídica e, usando as ferramentas, encontrar a legislação, doutrina e jurisprudência para responder à questão.
        ...
        ```json
        {{"fundamentos_legais": [{{"lei": "Nome da Lei/Código", "artigos": "Artigos relevantes", "descricao": "Descrição da relevância dos artigos para a consulta."}}], "principios_juridicos": ["Princípios que norteiam a discussão."], "jurisprudencia_relevante": "Cite uma decisão ou entendimento encontrado com as ferramentas que seja crucial para responder à consulta.", "analise_juridica_detalhada": "Análise técnica que conecta os fundamentos para formar o raciocínio central do parecer."}}
        ```
        Comece!

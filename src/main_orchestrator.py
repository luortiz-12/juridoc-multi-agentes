...
class Orquestrador:
    ...
    def gerar_documento(self, raw_input_data: dict) -> dict:
        ...
        for tentativa in range(max_tentativas):
            ...
            if "Tipo de documento não suportado" in documento_gerado:
            ...
            if resultado_validacao.get("status") == "aprovado":
                ...
                # Isso é uma simplificação. Em um sistema mais complexo, a sugestão seria mais direcionada.
...
if __name__ == '__main__':
    ...
    if not openai_api_key:
        print("Erro: Chave da API OpenAI não encontrada. Por favor, defina a variável de ambiente OPENAI_API_KEY.")
        exit(1)
    ...
    # Usando o exemplo de petição do arquivo pasted_content.txt
    raw_input_data_peticao = {
        "body": {
            "tipoDocumento": "peticao",
            "tipoContrato": None, # Não aplicável para petição
    ...
    # Simulação de uma entrada de dados para contrato
    ...
    if resultado_peticao["status"] == "sucesso":
        ...
        print(f"Detalhes: {resultado_peticao['detalhes']}")
    ...
    if resultado_contrato["status"] == "sucesso":
        ...
        print(f"Detalhes: {resultado_contrato['detalhes']}")

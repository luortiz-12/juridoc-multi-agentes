...
class AgenteRedatorTrabalhista:
    ...
    async def gerar_documento_html_puro_async(self, dados_formulario: Dict, pesquisas: Dict, documento_anterior: Optional[str] = None, recomendacoes: Optional[List[str]] = None) -> str:
        ...
        instrucao_referencia = "IMPORTANTE: Após a qualificação inicial das partes no início do documento, refira-se a elas apenas pelo nome e pela sua condição (ex: 'o Reclamante' ou 'a Reclamada'). NÃO repita a qualificação completa (CPF, RG, nacionalidade, etc.) no corpo do texto, nunca coloque  qualificação comleta no inicio dos FATOS ou em PEDIDOS ."
